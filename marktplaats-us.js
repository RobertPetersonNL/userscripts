// ==UserScript==
// @name         Tweedehandsplaats
// @version      1.0.4
// @namespace    Marktplaats
// @description  Verwijderd promoties, ads en bedrijven op marktplaats
...
(() => {
    ...
    const hideAds = () => {
        document.querySelectorAll('[class*="Listing--list-item"]').forEach(item => {
            ...
            const categoryElement = item.querySelector('[class*="Listing-category-name"]');

            // 2. Haal de tekst eruit
            const sellerName = seller?.textContent?.trim() ?? '';
            const categoryName = categoryElement?.textContent?.trim() ?? '';

            // 3. Definieer de filters
            const forbiddenCategories = ['Boeken', 'Antiek | Boeken'];
            const keywords = [
                'Bezorgt in',
                'Topadvertentie',
                'Dagtopper',
